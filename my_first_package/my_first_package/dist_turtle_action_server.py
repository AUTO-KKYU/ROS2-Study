'''
TurtleSub_Action 클래스
- TurtlesimSubscriber 클래스를 상속
- 거북이 시뮬레이션에서 발생한 데이터를 구독하는 역할
- callback 메서드는 거북이의 위치 메시지를 받아오는 역할
- 이 메시지를 Action Server로 전달합니다.

DistTurtleServer 클래스
- Node 클래스를 상속
- 거북이의 이동 거리를 추적하고, Action Server를 설정
- calc_diff_pose 메서드는 이전 위치와 현재 위치 사이의 거리를 계산하여 거리 차이를 반환
- execute_callback 메서드는 Action 요청을 처리하고, 거북이를 움직이며 거리를 측정
- 거리가 일정 값 미만이 되면 작업을 완료하고 결과를 반환합니다. 
'''

'''
TurtleSub_Action 클래스에서는 ac_server 매개변수로 DistTurtleServer 객체를 받아와서 이 객체를 활용하여 거북이의 위치 메시지를 Action Server로 전달
'''
...
class DistTurtleServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        while True:
            ...
            tmp = feedback_msg.remained_dist = goal_handle.request.dist * self.total_dist
            tmp = abs(tmp)
